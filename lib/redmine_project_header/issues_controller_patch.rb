module RedmineProjectHeader
  ...
  module IssuesControllerPatch
    ...
    def self.prepended(base)
      ...
      Rails.logger.debug("#{LOGGER_TAG} IssuesController patched — X-Project header enabled") rescue nil
    end
    ...
    def append_project_header
      if @project.present?
        ...
        Rails.logger.debug("#{LOGGER_TAG} Set X-Project: \"#{@project.name}\" " \
        ...
        Rails.logger.debug("#{LOGGER_TAG} Set X-Project: \"#{@issue.project.name}\" via @issue " \
                           "(action: #{action_name}, path: #{request.path})")
      else
